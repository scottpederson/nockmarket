...
var envHost = process.env['MONGO_NODE_DRIVER_HOST']
  ,envPort = process.env['MONGO_NODE_DRIVER_PORT']
  ,host = envHost != null ? envHost: 'localhost'
//  ,port = envPort != null ? parseInt(envPort, 10) : '12345';
	,port = envPort != null ? envPort: '12345';
  ...
  open: function(callback) {
    db.open(function(err, data) {
      if (process.env.MONGO_NODE_DRIVER_USER) {
        data.authenticate(process.env.MONGO_NODE_DRIVER_USER, process.env.
				MONGO_NODE_DRIVER_PASS, function(err2, authData) {
